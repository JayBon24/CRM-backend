...
class UserInfoUpdateSerializer(CustomModelSerializer):
    ...
    def validate_mobile(self, value):
        ...
        if isinstance(value, str):
            ...
            if value == "":
                return value
        current_mobile = getattr(self.instance, "mobile", None)
        if current_mobile == value:
            return value
        queryset = Users.objects.filter(mobile=value)
        if self.instance is not None:
            queryset = queryset.exclude(pk=self.instance.pk)
        if queryset.exists():
            raise serializers.ValidationError("手机号必须唯一")
        return value
...
class ExportUserProfileSerializer(CustomModelSerializer):
    ...
    dept_name_all = serializers.SerializerMethodField(read_only=True)
    role_names = serializers.SerializerMethodField(read_only=True)
    role_level_display = serializers.SerializerMethodField(read_only=True)
    org_scope_display = serializers.SerializerMethodField(read_only=True)
    ...
    def get_role_names(self, instance):
        roles = instance.role.all()
        names = [r.name for r in roles]
        return ",".join(names)

    def get_role_level_display(self, instance):
        mapping = {
            "HQ": "总所管理",
            "BRANCH": "分所管理",
            "TEAM": "团队管理",
        ...
        return mapping.get(getattr(instance, "role_level", None), "")

    def get_org_scope_display(self, instance):
        mapping = {
            "HQ": "全所",
            "BRANCH": "本所",
            "TEAM": "本团队",
        ...
        return mapping.get(getattr(instance, "org_scope", None), "")
...
class UserProfileImportSerializer(CustomModelSerializer):
    ...
    def save(self, **kwargs):
        ...
        if raw_password is not None and str(raw_password).strip():
            # Excel 提供了密码：仅调用 set_password(明文)，内部会做一次 MD5 再 PBKDF2，不做双重 MD5
            data.set_password(str(raw_password).strip())
            data.save()
        else:
            # Excel 未提供密码（模板无密码列）：仅新建用户设置默认密码，更新用户不修改密码
            if self.instance is None:
                data.set_password("admin123456")
                data.save()
        return data
...
class UserViewSet(CustomModelViewSet):
    ...
    export_field_label = {
        ...
        "mobile": "手机号码",
        "gender": "用户性别",
        "user_type_display": "用户类型",
        "is_active": "账号状态",
        "dept_name_all": "部门",
        "role_names": "角色",
        "role_level_display": "角色层级",
        "org_scope_display": "数据范围",
    }
    ...
    import_field_dict = {
        ...
        "role_level": {
            "title": "角色层级",
            "choices": {
                "data": {"总所管理": "HQ", "分所管理": "BRANCH", "团队管理": "TEAM", "销售": "SALES"},
            }
        },
        "org_scope": {
            "title": "数据范围",
            "choices": {
                "data": {"全所": "HQ", "本所": "BRANCH", "本团队": "TEAM", "本人": "SELF"},
            }
        },
    }

    def filter_queryset_for_import_delete(self, queryset):
        """批量更新时「删除未在 Excel 中的账号」：不删超管、不删当前登录用户。"""
        qs = queryset.exclude(is_superuser=True)
        if hasattr(self, "request") and self.request and getattr(self.request, "user", None):
    ...
    def user_info(self, request):
        ...
        result = {
            ...
            "org_scope": getattr(user, 'org_scope', None),
            # 组织架构ID信息
            "headquarters_id": getattr(user, 'headquarters_id', None),
    ...
    def update_user_info(self, request):
        """修改当前用户信息"""
        serializer = UserInfoUpdateSerializer(request.user, data=request.data)
