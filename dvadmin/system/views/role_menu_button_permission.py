...
class RoleMenuButtonPermissionViewSet(CustomModelViewSet):
    ...
    def get_role_menu(self, request):
        ...
        serializer = RoleMenuSerializer(menu_queryset, many=True, context={'request': request})
    ...
    def set_role_menu(self, request):
        ...
        serializer = RoleMenuSerializer(menu_instance, context={'request': request})
    ...
    def get_role_menu_btn_field(self, request):
        ...
        menu_btn_serializer = RoleMenuButtonSerializer(menu_btn_queryset, many=True, context={'request': request})
        menu_field_queryset = MenuField.objects.filter(menu_id=menuId)
        menu_field_serializer = RoleMenuFieldSerializer(menu_field_queryset, many=True, context={'request': request})
    ...
    def set_role_menu_btn(self, request):
        ...
        serializer = RoleMenuButtonSerializer(menu_btn_instance, context={'request': request})
        return DetailResponse(data=serializer.data, msg="更新成功")
    ...
    def set_role_menu_btn_data_range(self, request):
        ...
        if not data.get('dept'):
            instance.dept.clear()
        instance.save()
        serializer = RoleMenuButtonPermissionSerializer(instance)
        return DetailResponse(data=serializer.data, msg="更新成功")
