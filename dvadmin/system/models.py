...
class Users(CoreModel, AbstractUser):
    ...
    wechat_openid = models.CharField(max_length=255, unique=True, null=True, blank=True, db_index=True, verbose_name="微信OpenID", help_text="微信小程序OpenID")
    wechat_unionid = models.CharField(max_length=255, null=True, blank=True, db_index=True, verbose_name="微信UnionID", help_text="微信UnionID")
    ...
    role_level = models.CharField(
        ...
        blank=True,
        verbose_name="角色层级",
        help_text="角色层级：HQ(总所管理)/BRANCH(分所管理)/TEAM(团队管理)/SALES(销售)"
    )
    ...
    org_scope = models.CharField(
        max_length=20,
        choices=ORG_SCOPE_CHOICES,
        null=True,
        blank=True,
        verbose_name="数据范围",
        help_text="数据范围：HQ(全所)/BRANCH(本所)/TEAM(本团队)/SELF(本人)"
    )
    
    # 组织架构关联字段（独立的组织架构表）
    headquarters_id = models.IntegerField(
        null=True,
        blank=True,
        verbose_name="所属总部ID",
        help_text="所属总部ID",
...
class FileList(CoreModel):
    ...
    file_blob = models.BinaryField(null=True, blank=True, verbose_name="文件二进制", help_text="文件二进制内容（db模式）")

    def save(self, *args, **kwargs):
        if (self.engine or '').lower() == 'db':
            if not self.file_url and self.pk:
                self.file_url = f'admin-api/system/file/{self.pk}/preview/'
            super(FileList, self).save(*args, **kwargs)
            if not self.file_url:
                self.file_url = f'admin-api/system/file/{self.pk}/preview/'
                super(FileList, self).save(update_fields=['file_url'])
            return
