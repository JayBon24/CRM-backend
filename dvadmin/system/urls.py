...
urlpatterns = [
    ...
    # 与前端/菜单按钮约定一致：导入模板下载和提交也支持 import_data/ 路径，避免 404
    path('user/import_data/', UserViewSet.as_view({'get': 'import_data', 'post': 'import_data'})),
    # 批量更新模板下载（与前端 api/system/user/update_template/ 一致）
    path('user/update_template/', UserViewSet.as_view({'get': 'update_template'})),
