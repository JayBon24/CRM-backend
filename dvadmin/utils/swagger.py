...
class CustomOpenAPISchemaGenerator(OpenAPISchemaGenerator):
    def get_schema(self, request=None, public=False):
        ...
        try:
            ...
            # 捕获 'NoneType' object has no attribute 'startswith' 错误
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Swagger schema generation error: {str(e)}", exc_info=True)
            ...
            swagger = openapi.Swagger(
                info=openapi.Info(
                    title="API",
                    default_version="v1",
                    description="API Documentation (部分接口因配置问题未显示，请检查日志)",
                ),
                paths={},
                _url='',
            )
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Unexpected swagger schema generation error: {str(e)}", exc_info=True)
            ...
            swagger = openapi.Swagger(
                info=openapi.Info(
                    title="API",
                    default_version="v1",
                    description=f"API Documentation (生成文档时出错: {str(e)})",
                ),
                paths={},
                _url='',
            )
