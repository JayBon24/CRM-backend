...
def _build_oss_bucket():
    ...
    if not all([access_key_id, access_key_secret, endpoint, bucket_name]):
        raise ValidationError('阿里云OSS配置不完整')
    if bucket_name.endswith(endpoint):
        bucket_name = bucket_name.replace(f'.{endpoint}', '')
    auth = oss2.Auth(access_key_id, access_key_secret)
...
def aliyun_oss_extract_key(file_path_or_key: str) -> str:
...
def ali_oss_delete(file_path_or_key: str) -> bool:
    key = aliyun_oss_extract_key(file_path_or_key)
    if not key:
        return False
    try:
        bucket = _build_oss_bucket()
        result = bucket.delete_object(key)
        return int(getattr(result, 'status', 500)) < 300
    except Exception:
        return False
